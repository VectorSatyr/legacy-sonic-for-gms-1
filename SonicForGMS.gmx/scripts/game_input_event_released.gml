/// game_input_event_released(event, [channel])
/**
 * @description Checks if the given event was registered last frame and not this frame
 * @argument {string} event event name
 * @argument {real} channel (optional) input channel; if unused, polls all available channels
 * @returns {boolean} whether or not the event was registered
 */

var channel = -1;
switch (argument_count) {
case 2: channel = argument[1];
default:
    var event = argument[0];
}

var result = false;

with (InputSource) {
    if (channel == -1 or self.channel == channel) {
        if (ds_list_find_index(events, event) == -1 and 
            ds_list_find_index(previous_events, event) != -1) {
            result = true;
        }
    }
}

return result;
