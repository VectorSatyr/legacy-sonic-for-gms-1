/// game_pc_calc_shape_normal(character, shape, orientation)
...
with (character) {
    if (game_pc_in_shape_bounds(self, shape, orientation)) {
        if (shape.surface_direction > -1) {
            normal = shape.surface_direction; // hard-coded
            break;
        }
        if (object_is_family(kind, Rectangle)) {
            break; // default
        }
        if (object_is_family(kind, RightTriangle)) {
            var x1 = shape.bbox_left;
            var y1 = shape.bbox_bottom + 1;
            var x2 = shape.bbox_right + 1;
            var y2 = shape.bbox_top;
        ...
        if (object_is_family(kind, QuarterPipe)) {
            if (xrad == yrad) {
                ang = point_direction(shape.x, shape.y, ox, oy);
            } else {
                var ox2 = ox - shape.x;
        ...
        if (total > 0) {
            ...
            repeat (offset) { //y_radius + offset
                ...
                for (n = 0; n < total; ++n) {
                    ...
                    if (ind.top_solid) {
                        if (not left and game_shape_in_point(ind, x1, y1)) {
                            left = ind; // left sensor height
                        }
                        if (not right and game_shape_in_point(ind, x2, y2)) {
                            right = ind; // right sensor height
                        }
                    }
                }
                if (left and right) {
